...
router.post('/game/:imageId/solved', function *(next) {
  ...
  yield showGame(this, this.params.imageId, true /* showOriginalImage*/, '我用' + this.request.body.step + '步拼出了这张图，奔走相告小伙伴！');
});
...
router.get('/hot', function *(next) {
  ...
  var hots = [];
  for (var i = 0; i < imageIds.length; i++) {
    var imageMetadata = image_metadata.getMetadata(imageIds[i]);
    hots.push({id: imageIds[i], 
      imageMetadata: imageMetadata,
      url: router.url(GAME_URL_NAME, imageIds[i])});
  }
  yield this.render(pageName, {
...
var showHotImage = function *(context, imageIds, user) {
  }
