...
function createCommentNotification(sender, imageId, comment) {
  return {
    type: TYPE_COMMENT,
    sender: sender,
    comment: comment,
...
module.exports = {
  ...
  addSolvedNotification: function *(userId, sender, imageId, stepCount) {
    var newItem = createSolvedNotification(sender, imageId, stepCount);
    var notifi = yield addNewNotification(this, userId, newItem);
    return notifi;
  },
  addCommentNotification: function *(userId, sender, imageId, comment) {
    logger.info('imageid is ' + imageId);
    var newItem = createCommentNotification(sender, imageId, comment);
    var notifi = yield addNewNotification(this, userId, newItem);
    return notifi;
  },
